...
const App = () => {
  ...
  const [textToType, setTextToType] = useState(() => ArrayOfSentences[Math.floor(Math.random() * 9)]);
  const [visibleCharsCount, setVisibleCharsCount] = useState(symbolInFourSentences);
  const [isFocused, setIsFocused] = useState(true);
  const inputRef = useRef<HTMLInputElement>(null);
  const debouncedFunc = useRef(Debounce(setInputValue, 8)).current;
  ...
  function handleRestart() {
    setTextToType(ArrayOfSentences[Math.floor(Math.random() * 9)]);
    debouncedFunc("");
  }
